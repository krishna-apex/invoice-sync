...
def generate_invoice_pdf(invoice, client, time_entries, user_email, out_dir="pdfs"):
    ...
    try:
        ...
        due_str = ""
    info_block = f'<b>Period</b><br/>{invoice["period_start"]} to {invoice["period_end"]}<br/><br/><b>Issued</b><br/>{invoice["created_at"][:10]}<br/><br/><b>Due</b><br/>{due_str or "on receipt"}<br/><br/><b>Amount</b><br/><font size=13 color="#2563EB"><b>{client["currency"]} {float(invoice["amount"]):.2f}</b></font>'
    ...
    if time_entries:
        ...
        data.append([
            Paragraph("", s_small), Paragraph("Subtotal", s_small), Paragraph(f"{total_hrs:.2f}h", s_small), Paragraph("", s_small), Paragraph(f"{currency} {subtotal:.2f}", s_small),
        ])
        if taxp:
            data.append([
                Paragraph("", s_small), Paragraph(f"Tax ({taxp:g}%)", s_small), Paragraph("", s_small), Paragraph("", s_small), Paragraph(f"{currency} {taxamt:.2f}", s_small),
            ])
        data.append([
